...
class Process(object):
    ...
    def addFeature(self):
        ...
        self.data.loc[self.data['realbody'] <= 0, 'ushadow_width'] = self.data.loc[self.data['realbody'] <= 0, 'high'] - \
                                                                     self.data.loc[self.data['realbody'] <= 0, 'open']
        self.data.loc[self.data['realbody'] > 0, 'ushadow_width'] = self.data.loc[self.data['realbody'] > 0, 'high'] - \
                                                                    self.data.loc[self.data['realbody'] > 0, 'close']
        self.data.loc[self.data['realbody'] <= 0, 'lshadow_width'] = self.data.loc[
                                                                         self.data['realbody'] <= 0, 'close'] - \
                                                                     self.data.loc[self.data['realbody'] <= 0, 'low']
        self.data.loc[self.data['realbody'] > 0, 'lshadow_width'] = self.data.loc[self.data['realbody'] > 0, 'open'] - \
                                                                    self.data.loc[self.data['realbody'] > 0, 'low']
